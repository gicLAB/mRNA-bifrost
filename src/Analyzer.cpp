...
void mrna::Analyzer::parseEnergyPara(std::ifstream& infile) {
  std::string buffer;
...
void mrna::Analyzer::parseconfig(std::ifstream& infile) {
  std::string buffer;
  while(getline(infile, buffer)) {
    std::istringstream record(buffer);
    std::string str;
...
int mrna::Analyzer::CalculateCNNConfig1_Inst(int kernelchannel, int kernelfilters) {
   int instnum = 0;
...
int mrna::Analyzer::CalculateCNNPartial_Inst(int config2vnnum) {
  int instlength = PUSH_LENGTH + PULL_LENGTH;
  return instlength;
}

int mrna::Analyzer::CNNConfig1largefil_Inst(int kernel_y, int kernelfilters) {
    int num = 0;
    if(dnn_model->cnn_filter->window_stride == 1) {
...
double mrna::Analyzer::CalculateConfig1_UR(int cycle) {
...
void mrna::Analyzer::CalculateConfig1onchip(MappingStrategy* map, std::vector<double> partial, int type) {
  if(type == 0) {
...
double mrna::Analyzer::CalculatePartialsum(int psnum) {
  double sum = dnn_model->cnn_output->output_x * dnn_model->cnn_output->output_y * dnn_model->cnn_output->output_channel * psnum * 2 / 1024;
  return sum;
}

void mrna::Analyzer::CalculateEnergy(MappingStrategy* map, int config_num) {
...
/*
void mrna::Analyzer::CalculateCycle(MappingStrategy* map, int config_num) {
...
void mrna::Analyzer::writeProfile(std::ofstream& profile, std::map<int, MappingStrategy*> mappings) {
...
bool mrna::Analyzer::checkCNNInput(){
  bool correct = false;
...
void mrna::Analyzer::CalculateDSN(MappingStrategy* map, int confignum, int type) {
...
void mrna::Analyzer::CalculateMSN(MappingStrategy* map, int confignum, int type){
...
void mrna::Analyzer::CalculateRSN(MappingStrategy* map, int confignum) {
...
void mrna::Analyzer::CalculateSPM(MappingStrategy* map, int confignum, int type) {
...
void mrna::Analyzer::CalculateDram(MappingStrategy* map, std::vector<double> partialsum, int type) {
...
bool mrna::Analyzer::compMappingcycle(MappingStrategy*& m1, MappingStrategy*& m2) {
  const unsigned long long c1 = m1->gettotal_cycle();
  const unsigned long long c2 = m2->gettotal_cycle();
  return c1 < c2;
}

bool mrna::Analyzer::compMappingEnergy(MappingStrategy* m1, MappingStrategy* m2) {
  const double e1 = m1->gettotal_energy();
  const double e2 = m2->gettotal_energy();
  return e1 < e2;
}

bool mrna::Analyzer::compMappingEE(MappingStrategy* m1, MappingStrategy* m2) {
  const double ee1 = m1->getenergy_efficiency();
  const double ee2 = m2->getenergy_efficiency();
  return ee1 > ee2;
}

mrna::MappingStrategy* mrna::Analyzer::SortMappingStrategy(OptGoal goal) {
  ...
  if(goal == performance) {
      sortlist.sort(mrna::Analyzer::compMappingcycle);
  }
  else if(goal == energy) {
    sortlist.sort(mrna::Analyzer::compMappingEnergy);
  }
  else if(goal == energy_efficiency){
    sortlist.sort(mrna::Analyzer::compMappingEE);
  }
...
void mrna::Analyzer::AnalyzeCNN(std::ofstream& profile, OptGoal option) {
   if(!checkCNNInput()) {
...
int mrna::Analyzer::CalculateFCConfig1_Inst(int vn_num) {
  int inst_num = 0;
  if(vn_num > 1) {
...
int mrna::Analyzer::CalculateFCConfig2_Inst(int vn_num) {
  int inst_num = PUSH_LENGTH + PULL_LENGTH;
  return inst_num;
}

bool mrna::Analyzer::checkFCInput(){
  bool correct = false;
...
void mrna::Analyzer::AnalyzeFC(std::ofstream& profile, OptGoal option) {
...
bool mrna::Analyzer::checkRNNInput() {
  bool correct = false;
...
int mrna::Analyzer::CalculateRNNConfig1_Inst(int vn_num) {
  int inst_num = 0;
  if(vn_num > 1) {
...
int mrna::Analyzer::CalculateRNNConfig2_Inst(int vn_num) {
  int inst_num = PUSH_LENGTH * 2 + 2 * PULL_LENGTH ;
  return inst_num;
}

int mrna::Analyzer::CalculateLSTMCt_Inst(int stage, int vn_num) {
  int inst_num = 0;
  if (stage == 1) {
...
void mrna::Analyzer::AnalyzeRNN(std::ofstream& profile, OptGoal option) {
...
void mrna::Analyzer::writeCambrion(std::ofstream& profile, std::vector<std::pair<int, double> > info) {
  int i = 1;
...
void mrna::Analyzer::AnalyzeCambricon(std::ofstream& profile) {
...
void mrna::Analyzer::AnalyzeCambriconFC(std::ofstream& profile) {
  int filterx = dnn_model->cnn_filter->filter_x;
  int filtery = dnn_model->cnn_filter->filter_y;
  int filterc = dnn_model->cnn_filter->filter_channel;
  int filtern = dnn_model->cnn_filter->filter_number;
...
void mrna::Analyzer::writeSystolic(std::ofstream& profile, std::map<std::pair<int, int>, double> arraylist) {
...
void mrna::Analyzer::AnalyzeSystolicCNN(std::ofstream &profile) {
...
void mrna::Analyzer::AnalyzeSystolicFC(std::ofstream &profile) {
  int outputnum = 1;
...
void mrna::Analyzer::ConfigGen(std::ofstream& config) {
